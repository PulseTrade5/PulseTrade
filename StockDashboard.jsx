...
function fmtCr(n) {
  ...
  if (cr >= 1e3) return `â‚¹${(cr/1e3).toFixed(2)}K Cr`;
...
function TrendMeter({ longScore, shortScore, trend, C }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, letterSpacing: 2, color: C.muted, marginBottom: 8, fontWeight: 700 }}>ðŸŽ¯ TREND METER</div>
      ...
      <div style={{
        ...
        {score >= 70 ? 'ðŸ”¥ Strong ' : score >= 40 ? 'âš¡ Moderate ' : 'â„ï¸ Weak '}{trend}
      </div>
    </div>
  );
}
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 12, fontWeight: 800, padding: '6px 14px', borderRadius: 20, backgroundColor: accentColor, color: '#FFF' }}>
          {isBullish ? 'ðŸŸ¢ BULLISH' : 'ðŸ”´ BEARISH'}
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        {stockInfo?.regularMarketChange !== undefined && (
          <span style={{
            ...
            {stockInfo.regularMarketChange >= 0 ? 'â–²' : 'â–¼'} {Math.abs(stockInfo.regularMarketChangePercent || 0).toFixed(2)}%
          </span>
        )}
      </div>
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', fontWeight: 700, letterSpacing: 1 }}>âš¡ PULSE SCORE</span>
      ...
      <div style={{ fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.75)', textAlign: 'center', marginTop: 16, padding: '8px 12px', backgroundColor: 'rgba(255,255,255,0.07)', borderRadius: 10 }}>
        {isBullish ? score >= 70 ? 'ðŸ”¥ Strong Bullish â€” Momentum bahut strong hai!' : 'âš¡ Moderate Bullish â€” Cautiously optimistic raho'
          : score >= 70 ? 'â„ï¸ Strong Bearish â€” Selling pressure strong hai!' : 'âš¡ Moderate Bearish â€” Dhyan se dekho'}
      </div>
    </div>
  );
}
...
function AITradeCoach({ stockData, C, isDark }) {
  ...
  const analyzeChart = async () => {
    ...
    try {
      ...
      setChartAnalysis('Network error â€” dobara try karo!');
    }
  ...
  const askCoach = async () => {
    ...
    setAnswer('');
    try {
      const context = stockData ? `Stock: ${stockData.symbol}, Trend: ${stockData.trend}, RSI: ${stockData.rsi}, ADX: ${stockData.adx}, Pulse Score: ${stockData.trend === 'Bullish' ? stockData.longScore : stockData.shortScore}/100, Price: â‚¹${stockData.currentPrice}` : '';
      ...
      const data = await response.json();
      setAnswer(data.answer || 'Kuch gadbad ho gayi â€” dobara try karo!');
    } catch {
      setAnswer('Network error â€” dobara try karo!');
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div style={{ fontSize: 10, letterSpacing: 2, color: C.gold, fontWeight: 800 }}>ðŸ¤– AI TRADE COACH</div>
        <button onClick={() => setShowCoach(false)} style={{ background: 'none', border: 'none', color: C.muted, cursor: 'pointer', fontSize: 16 }}>âœ•</button>
      </div>

      {/* MODE SWITCHER */}
      <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        {[['text', 'ðŸ’¬ Sawaal Pucho'], ['chart', 'ðŸ“¸ Chart Analyze']].map(([m, l]) => (
          ...
          {chartImage && (
            <>
              ...
              <button onClick={analyzeChart} disabled={chartLoading} style={{
                ...
                {chartLoading ? 'â³ AI Analyze kar raha hai...' : 'ðŸ” Chart Analyze Karo'}
              </button>
            </>
          )}
          {chartAnalysis && (
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span style={{ color: C.gold, fontWeight: 700 }}>ðŸ¤– AI Analysis:</span>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <input
          ...
          placeholder="Kuch bhi pucho â€” Hinglish mein!"
          style={{
      ...
      <div style={{ fontSize: 10, color: C.muted, marginTop: 10, textAlign: 'center' }}>
        âš ï¸ Sirf educational â€” investment advice nahi
      </div>
    </div>
  );
}
...
function PulseOracle({ userDob, isDark, C }) {
  ...
  const getOracle = async () => {
    ...
    try {
      ...
      setOracle('Network error â€” dobara try karo!');
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div>
          <div style={{ fontSize: 10, letterSpacing: 2, color: '#7C3AED', fontWeight: 800 }}>ðŸ”® PULSE ORACLE</div>
      ...
      {!shown ? (
        <button onClick={getOracle} disabled={loading} style={{
          ...
          {loading ? 'ðŸ”® Oracle dekh raha hai...' : 'ðŸ”® Aaj Ka Prediction Dekho'}
        </button>
      ) : (
        <div>
          <div style={{
          ...
          }}>ðŸ”„ Dobara Dekho</button>
        </div>
      )}

      <div style={{ fontSize: 10, color: C.muted, marginTop: 10, textAlign: 'center' }}>
        âœ¨ Sirf entertainment â€” investment advice nahi
      </div>
    </div>
  );
}
...
function ReferralCard({ user, C }) {
  ...
  return (
    <div style={{ backgroundColor: C.surface, border: `1px solid ${C.surfaceBorder}`, borderRadius: 16, padding: 18, marginBottom: 16, boxShadow: '0 1px 6px rgba(0,0,0,0.05)' }}>
      <div style={{ fontSize: 10, letterSpacing: 2, color: C.muted, marginBottom: 14, fontWeight: 700 }}>ðŸ”— MERA REFERRAL LINK</div>
      <div style={{ fontSize: 12, color: C.muted, marginBottom: 12, lineHeight: 1.6 }}>
        Dost ko invite karo â†’ <span style={{ color: C.gold, fontWeight: 700 }}>tujhe +100 pts</span>, unhe <span style={{ color: C.green, fontWeight: 700 }}>+50 pts</span>! ðŸŽ
      </div>
      {refLink ? (
        <>
          ...
          <button onClick={handleCopy} style={{ width: '100%', padding: '11px', borderRadius: 10, border: 'none', backgroundColor: copied ? C.green : C.gold, color: '#FFF', fontWeight: 700, fontSize: 13, cursor: 'pointer', transition: 'background 0.3s' }}>
            {copied ? 'âœ… Link Copy Ho Gaya!' : 'ðŸ“‹ Link Copy Karo'}
          </button>
          <div style={{ textAlign: 'center', fontSize: 12, color: C.muted, marginTop: 10 }}>
            ðŸŽ¯ <span style={{ color: C.gold, fontWeight: 700 }}>{refCount}</span> dost join kiya abhi tak!
          </div>
        </>
      ) : (
        <div style={{ textAlign: 'center', fontSize: 12, color: C.muted, padding: '10px 0' }}>â³ Referral code load ho raha hai...</div>
      )}
    </div>
  );
}
...
export default function StockDashboard({ user, isDark, onTabChange, defaultTab }) {
  ...
  const startVoiceSearch = () => {
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
      alert('Tera browser voice search support nahi karta â€” Chrome use karo!');
      return;
    }
    ...
    <div style={{ backgroundColor: C.bg, color: C.text, minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif', transition: 'all 0.3s ease' }}>
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 0 48px' }}>
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 11, color: C.gold, fontWeight: 700, letterSpacing: 0.3 }}>à¤¶à¥à¤°à¥€ à¤—à¤£à¥‡à¤¶à¤¾à¤¯ à¤¨à¤®à¤ƒ</div>
            <h1 style={{ fontSize: 22, fontWeight: 800, margin: 0, letterSpacing: '-0.5px' }}>
              Pulse<span style={{ color: C.gold }}>Trade</span>
            </h1>
            <div style={{ fontSize: 10, color: C.muted, marginTop: 1 }}>ðŸ”± à¤¹à¤° à¤¹à¤° à¤®à¤¹à¤¾à¤¦à¥‡à¤µ ðŸ”±</div>
          </div>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 18, flexShrink: 0, marginTop: 1 }}>ðŸ›¡ï¸</span>
          <div>
            <div style={{ fontSize: 12, fontWeight: 800, color: C.sebi, marginBottom: 3 }}>SEBI DISCLAIMER â€” ZAROORI PADHE</div>
            <div style={{ fontSize: 11.5, color: C.sebi, lineHeight: 1.6, opacity: 0.85 }}>
              Yeh platform sirf <strong>technical trend analysis</strong> provide karta hai â€” yeh <strong>investment advice nahi hai</strong>. All outputs are algorithmic technical-analysis estimates for educational and informational purposes only. SEBI-registered advisor se salah zaroor lein.
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: '20px 20px 0' }}>
          ...
          <div style={{
            ...
            {[['check','ðŸ” Check'],['watchlist','â­ Watchlist'],['track','ðŸ“‹ Record'],['screener','ðŸš€ Screener']].map(([key,label]) => (
            ...
            <>
              ...
              <div style={cardStyle}>
                ...
                <div style={{ marginTop: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      {[['risk','Risk â‚¹'],['manual','Qty']].map(([m,l]) => (
                ...
                <button onClick={() => handleSearch()} disabled={loading} style={{
                  ...
                  {loading ? 'â³ Check ho raha hai...' : 'ðŸ” Trend Nikalo'}
                </button>
              ...
              {result && (
                <>
                  ...
                  {stockInfo && (
                    <div style={cardStyle}>
                      <div style={{ fontSize: 10, letterSpacing: 2, color: C.muted, marginBottom: 10, fontWeight: 700 }}>ðŸ“Š STOCK INFO</div>
                      {stockInfo.longName && (
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                            ...
                            {stockInfo.regularMarketChange !== undefined && (
                              <span style={{
                                ...
                                {stockInfo.regularMarketChange >= 0 ? 'â–²' : 'â–¼'} {fmtINR(Math.abs(stockInfo.regularMarketChange))} ({Math.abs(stockInfo.regularMarketChangePercent || 0).toFixed(2)}%)
                              </span>
                            )}
                          </div>
                        </div>
                  ...
                  <div style={cardStyle}>
                    <div style={{ fontSize: 10, letterSpacing: 2, color: C.muted, marginBottom: 14, fontWeight: 700 }}>ðŸ“ˆ INDICATOR STRENGTH</div>
                    <IndicatorBar label={`RSI â€” ${result.rsi > 70 ? 'Overbought' : result.rsi < 30 ? 'Oversold' : 'Neutral zone'}`} value={result.rsi} max={100} color={result.rsi > 70 ? C.red : result.rsi < 30 ? C.green : C.gold} C={C} />
                    <IndicatorBar label={`ADX â€” ${result.trendStrength}`} value={result.adx} max={60} color={result.adx >= 25 ? C.green : C.muted} C={C} />
                  ...
                  <div style={cardStyle}>
                    ...
                    <button onClick={() => {
                      ...
                      {watchlist.some(w => w.symbol === stockName) ? 'â­ Watchlist se hatao' : 'â˜† Watchlist mein add karo'}
                    </button>
                    <button onClick={() => {
                      ...
                      const change = stockInfo?.regularMarketChangePercent ? `${stockInfo.regularMarketChangePercent >= 0 ? 'â–²' : 'â–¼'} ${Math.abs(stockInfo.regularMarketChangePercent).toFixed(2)}%` : '';
                      const msg = `ðŸ”± *PulseTrade Analysis*

ðŸ“Š *${stockName}* â€” ${result.trend === 'Bullish' ? 'ðŸŸ¢ BULLISH' : 'ðŸ”´ BEARISH'}
ðŸ’° Price: â‚¹${Number(price).toLocaleString('en-IN', {maximumFractionDigits: 2})} ${change}
âš¡ Pulse Score: ${score}/100
ðŸ“ˆ RSI: ${result.rsi} | ADX: ${result.adx}
ðŸŽ¯ Trend: ${result.trendStrength}

${result.trend === 'Bullish' ? 'âœ… Strong Bullish Trend dikh raha hai!' : 'âš ï¸ Bearish Trend â€” Cautious raho!'}

ðŸ” Khud check karo: pulsetrade.in
ðŸ”± à¤¹à¤° à¤¹à¤° à¤®à¤¹à¤¾à¤¦à¥‡à¤µ ðŸ”±`;
                      ...
                      <span style={{fontSize: 16}}>ðŸ“±</span> WhatsApp Pe Share Karo
                    </button>
                  </div>
                  ...
                  {result.signal ? (
                    <div style={{
                      ...
                      <div style={{
                        ...
                        {result.signal==='LONG' ? 'ðŸ“ˆ Bullish Setup' : 'ðŸ“‰ Bearish Setup'}
                      </div>
                      {[
                        ['Entry', fmtINR(result.entry)],
                        ['Stop Loss', fmtINR(result.stopLoss)],
                        ['Safe Exit ðŸŸ¢ (3%)', fmtINR(result.targets?.[0])],
                        ['Sweet Spot ðŸŽ¯ (6%)', fmtINR(result.targets?.[1])],
                        ['Full Target ðŸš€ (10%)', fmtINR(result.targets?.[2])],
                      ...
                      <button onClick={handleSendAlert} disabled={alertSending || alertSent} style={{
                        ...
                        {alertSent ? 'âœ… Alert Bhej Diya!' : alertSending ? 'ðŸ“¨ Bhej rahe hain...' : 'ðŸ“§ Email Alert Bhejo'}
                      </button>
                    </div>
                  ) : (
                    <div style={{
                      ...
                      â³ Abhi koi clear confluence signal nahi hai. Wait karo.
                    </div>
                  )}
                  ...
                  <div style={cardStyle}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                      {[['BUY','ðŸ“ˆ Bullish Scenario'],['SELL','ðŸ“‰ Bearish Scenario']].map(([d, label]) => (
                      ...
                      {tierResults.map((t, i) => (
                        <div key={t.percent} style={{
                          ...
                          <div style={{ fontSize: 10, color: C.goldDim, fontWeight: 700 }}>
                            {i === 0 ? 'Safe Exit ðŸŸ¢' : i === 1 ? 'Sweet Spot ðŸŽ¯' : 'Full Target ðŸš€'}
                          </div>
                ...
                {watchlist.length === 0 ? (
                  ...
                  <div key={w.symbol} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: `1px solid ${C.surfaceBorder}` }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <button onClick={() => setWatchlist(prev => prev.filter(x => x.symbol !== w.symbol))} style={{ fontSize: 12, padding: '7px 12px', borderRadius: 8, border: `1.5px solid ${C.surfaceBorder}`, backgroundColor: 'transparent', color: C.red, cursor: 'pointer', fontWeight: 700 }}>âœ•</button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {history.length === 0 ? (
                ...
                <div key={h.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: `1px solid ${C.surfaceBorder}` }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>{fmtINR(h.price)} â€¢ {h.trend}</div>
                  </div>
                  <div style={{ display: 'flex', gap: 6 }}>
