  ...
  return (
    <div style={{ background: BG_CARD, border: `1px solid ${BORDER}`, borderRadius: "12px", padding: "20px", marginBottom: "20px", position: "relative", overflow: "hidden" }}>
      ...
      <button onClick={generateSummary} disabled={loading || !stockData?.symbol}
        style={{ width: "100%", padding: "12px 20px", background: loading ? "#30363D" : `linear-gradient(135deg, ${GOLD}, #B8860B)`, color: loading ? "#8B949E" : "#0D1117", border: "none", borderRadius: "8px", fontSize: "15px", fontWeight: 700, cursor: loading || !stockData?.symbol ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
        {loading ? <>⏳ AI soch raha hai...</> : <>🔊 Pulse Bolta Hai</>}
      </button>
      {summary && (
        <div style={{ marginTop: "16px", padding: "14px", background: "#0D1117", borderRadius: "8px", border: `1px solid ${speaking ? GOLD : "#30363D"}`, transition: "border-color 0.3s" }}>
          <p style={{ margin: 0, color: "#E6EDF3", fontSize: "14px", lineHeight: "1.7" }}>{summary}</p>
          <p style={{ margin: "10px 0 0", color: "#6E7681", fontSize: "11px", fontStyle: "italic" }}>⚠️ Sirf technical analysis hai — investment advice nahi. SEBI registered advisor se salah lein.</p>
        </div>
      )}
      {summary && (
      ...
      {error && <div style={{ marginTop: "12px", padding: "10px", background: "#1A0F0F", border: "1px solid #6E1B1B", borderRadius: "6px", color: "#FF6B6B", fontSize: "13px" }}>⚠️ {error}</div>}
