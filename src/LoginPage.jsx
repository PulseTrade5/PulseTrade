    ...
    <div style={{ backgroundColor: COLORS.bg, minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif', color: COLORS.text }}>
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 0 48px' }}>
        ...
        <div style={{ padding: '20px 20px 0' }}>
          <div style={{ ...cardStyle, textAlign: 'center', padding: '28px 20px', background: `linear-gradient(135deg, #ffffff 0%, ${COLORS.goldLight} 100%)`, border: `1.5px solid #f0c040`, boxShadow: '0 4px 24px rgba(200,146,10,0.12)' }}>
            ...
            <p style={{ fontSize: 13, color: COLORS.muted, margin: '0 0 16px', lineHeight: 1.6 }}>
              NSE/BSE stocks ka technical analysis — AI-powered Hinglish mein. Instant trend, entry, stop loss, targets.
            </p>
          ...
          <div style={{ ...cardStyle, border: `2px solid ${COLORS.gold}`, boxShadow: '0 4px 20px rgba(200,146,10,0.18)', textAlign: 'center' }}>
            <div style={{ display: 'inline-block', fontSize: 11, fontWeight: 800, letterSpacing: 1, color: '#FFF', backgroundColor: COLORS.red, padding: '4px 14px', borderRadius: 20, marginBottom: 10 }}>🔥 LIMITED TIME OFFER</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: COLORS.text, marginBottom: 4 }}>5-Din FREE Trial 🎉</div>
            <div style={{ fontSize: 12, color: COLORS.muted, marginBottom: 12 }}>Aaj signup karo — pura access 5 din ke liye bilkul free</div>
            <div style={{ fontSize: 14, fontWeight: 700, color: COLORS.goldDim, backgroundColor: COLORS.goldLight, display: 'inline-block', padding: '7px 18px', borderRadius: 10, marginBottom: 6, fontFamily: 'monospace', letterSpacing: 1 }}>
          ...
          <div style={cardStyle}>
            ...
            {sent ? (
              <div style={{ backgroundColor: COLORS.greenLight, border: `1.5px solid #bbf7d0`, borderRadius: 12, padding: '18px 16px', textAlign: 'center' }}>
                <div style={{ fontSize: 32, marginBottom: 8 }}>📧</div>
                <div style={{ fontSize: 15, fontWeight: 800, color: COLORS.green, marginBottom: 6 }}>Login link bhej diya!</div>
                <div style={{ fontSize: 13, color: COLORS.muted, lineHeight: 1.6 }}>
                  <strong>{email}</strong> pe ek magic link gaya hai.<br />
                  Apna email check karo aur link pe click karo.
                </div>
                <button onClick={() => { setSent(false); setEmail(''); }} style={{ marginTop: 14, fontSize: 12, color: COLORS.muted, background: 'transparent', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>
                  Doosra email try karo
                </button>
              </div>
            ) : (
              <>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} placeholder="tumhara@email.com" style={inputStyle} />
                {error && <p style={{ fontSize: 12, color: COLORS.red, marginTop: 6, fontWeight: 600 }}>{error}</p>}
                <button onClick={handleLogin} disabled={loading} style={{ width: '100%', marginTop: 12, padding: '14px', fontSize: 15, fontWeight: 700, borderRadius: 12, border: 'none', backgroundColor: loading ? '#CBD5E1' : COLORS.gold, color: loading ? COLORS.muted : '#FFF', cursor: loading ? 'not-allowed' : 'pointer', boxShadow: loading ? 'none' : '0 2px 14px rgba(200,146,10,0.35)' }}>
          ...
          <div style={cardStyle}>
            <div style={{ fontSize: 10, letterSpacing: 2, color: COLORS.muted, fontWeight: 700, marginBottom: 14 }}>⚡ KAISE KAAM KARTA HAI</div>
            {[
              ...
              <div key={item.title} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', padding: '10px 0', borderBottom: `1px solid ${COLORS.surfaceBorder}` }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, backgroundColor: COLORS.goldLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{item.emoji}</div>
                <div>
          ...
          <div style={cardStyle}>
            ...
            <p style={{ fontSize: 11, color: COLORS.muted, textAlign: 'center', marginTop: 12 }}>Pehle 5 din free trial karo — phir decide karo 😊</p>
          </div>
          ...
          <p style={{ textAlign: 'center', marginTop: 16, fontSize: 12, color: '#94A3B8' }}>🔱 हर हर महादेव 🔱</p>
        </div>
      </div>
    </div>
  );
