...
export default function PulseBoltaHai({ stockData }) {
  ...
  return (
    <div style={{ background: BG_CARD, border: `1px solid ${BORDER}`, borderRadius: "12px", padding: "20px", marginBottom: "20px", position: "relative", overflow: "hidden" }}>
      ...
      {summary && (
        <div style={{ display: "flex", gap: "10px", marginTop: "12px" }}>
          <button
            ...
            style={{ flex: 1, padding: "8px", background: speaking ? "#21262D" : "#1C2128", color: speaking ? GOLD : "#8B949E", border: `1px solid ${speaking ? GOLD : "#30363D"}`, borderRadius: "6px", fontSize: "13px", cursor: "pointer", fontWeight: 600 }}
          >
            {speaking ? "⏹ Rokho" : "🔁 Phir Sunao"}
          </button>
          <button
            onClick={() => { setSummary(""); stopSpeech(); }}
            style={{ padding: "8px 14px", background: "#21262D", color: "#8B949E", border: "1px solid #30363D", borderRadius: "6px", fontSize: "13px", cursor: "pointer" }}
          >
            ✕
          </button>
        </div>
      ...
      {error && (
        <div style={{ marginTop: "12px", padding: "10px", background: "#1A0F0F", border: "1px solid #6E1B1B", borderRadius: "6px", color: "#FF6B6B", fontSize: "13px" }}>
          ⚠️ {error}
        </div>
      )}
      <style>{`
